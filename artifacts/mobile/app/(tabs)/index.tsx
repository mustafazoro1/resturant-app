...
export default function HomeScreen() {
  ...
  const zingerBurger =
    ...
    menuItems.find((i) => i.id === "bu1") ??
    MENU_ITEMS.find((i) => i.id === "bu1");
  ...
  // Generate deals from menu items with active offers
  const generateDeals = () => {
  ...
  const dealsToShow = generateDeals().length > 0 ? generateDeals() : DEALS;
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <LinearGradient
        colors={["#0A2010", colors.darkGreen, colors.primary]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
        ...
        >
          <View style={styles.headerTop}>
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                user ? router.push("/(tabs)/profile") : router.push("/login")
              }
            >
              {user ? (
                user.profilePicUrl ? (
          ...
          >
            {dealsToShow.map((deal, idx) => (
              ...
              >
                <DealCard
                  deal={deal}
                  onPress={(d) => router.push(`/item/${d.itemId}`)}
                />
              </Animated.View>
            ))}
          </ScrollView>
          ...
          >
            {popularItems.map((item) => (
              <FoodCard
                key={item.id}
                item={item}
                onPress={(i: MenuItem) => router.push(`/item/${i.id}`)}
                horizontal={false}
              />
            ))}
          </ScrollView>
